...
export function EpubViewer({
  ...
  function nextPage() {
    turnPage(() => renditionRef.current?.next());
  }
  function prevPage() {
    turnPage(() => renditionRef.current?.prev());
  }

  /**
   * epub.js swaps its iframe content instantly with no transition. To get a
   * book-like "page swap" feel without fighting epub.js's internal DOM, we
   * briefly fade + slide the whole viewer out, swap the page underneath,
   * then fade it back in — cheap, but reads as a deliberate page turn.
   */
  function turnPage(advance: () => void) {
    const el = viewerRef.current;
    if (!el) {
      advance();
      return;
    }
    el.style.transition = "opacity 90ms ease-out, transform 90ms ease-out";
    el.style.opacity = "0";
    el.style.transform = "scale(0.985)";
    window.setTimeout(() => {
      advance();
      requestAnimationFrame(() => {
        el.style.transition = "opacity 160ms ease-in, transform 160ms ease-in";
        el.style.opacity = "1";
