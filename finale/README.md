...
## No login required (testing build)

Auth has been fully bypassed for local testing — there's no sign-in page,
and the app never needs to reach an auth endpoint:

- **Frontend**: `lib/auth-context.tsx` no longer calls the backend at all.
  It just provides a fixed demo user (`Demo Reader`) directly in the app,
  with no network round-trip.
- **Backend**: `JwtAuthGuard` no longer checks bearer tokens. Every request
  is treated as the same seeded demo account. Run this once so that account
  exists:
  ```bash
  cd backend
  npm run prisma:seed
...
Start both servers as usual and open `http://localhost:3000` — you land
straight in the library, no login call, no token, no `/login` route.

To restore real authentication for a production deployment:
1. Revert `backend/src/common/guards/jwt-auth.guard.ts` to
   `export class JwtAuthGuard extends AuthGuard('jwt') {}`.
2. Bring back `frontend/app/login/page.tsx` and `app/register/page.tsx`,
   and restore `login()`/`register()`/`api.me()`-based state in
   `lib/auth-context.tsx` (the original versions used `api.login`,
   `api.register`, and token storage in `lib/api.ts`, which are all still
