...
## No sign-in page (testing build)

The `/login` and `/register` pages have been removed from this build. Every
session automatically signs in as a fixed demo account instead:

1. In `/backend`, after `npx prisma migrate dev`, run:
   ```bash
   npm run prisma:seed
   ```
   This upserts the demo user: `demo@example.com` / `password123`.
2. Run both servers as usual (`npm run start:dev` / `npm run dev`) and open
   `http://localhost:3000` — it goes straight to `/library` as "Demo Reader",
   no form to fill in.

This still goes through the real auth flow (real JWT, real DB user) — the
forms are just gone. To bring sign-in/sign-up back for a real deployment,
you'd re-add `app/login/page.tsx` and `app/register/page.tsx` and swap the
auto-login in `lib/auth-context.tsx` back for a redirect-when-signed-out.
