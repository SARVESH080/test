export interface BookParagraph {
  /** Plain text content of the paragraph (empty for image paragraphs) */
  text: string;
  ...
  isHeading: boolean;
  /** Present when this "paragraph" is an image extracted from the PDF rather than text. */
  image?: {
    /** data: URL with the cropped image bytes */
    src: string;
    width: number;
    height: number;
  };
