...
export function ReaderShell({ bookId }: { bookId: string }) {
  ...
  // Auto-hide chrome after a moment of inactivity — but only while in
  // fullscreen. Outside fullscreen the toolbar should stay put; auto-hiding
  // it during normal windowed reading just makes it feel like it's
  ...
  // Re-evaluate whenever fullscreen state changes: entering fullscreen should
  // start the auto-hide countdown, exiting it should keep the chrome shown.
  useEffect(() => {
    bumpChrome(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isFullscreen]);
            ...
            {book.paragraphs.map((p, i) =>
              p.image ? (
                <figure
                  key={i}
                  data-p={i}
                  className="mb-[1em] flex break-inside-avoid justify-center"
                >
                  <img
                    src={p.image.src}
                    alt=""
