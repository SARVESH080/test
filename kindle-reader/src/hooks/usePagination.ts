...
export function usePagination({ deps }: UsePaginationOptions): UsePaginationResult {
  ...
  const measure = useCallback(() => {
    ...
    // Clip exactly at this page's box. Relying on the frame's own
    // overflow-hidden isn't enough: the frame clips at its own padded edge,
    // which sits further right/down than where this page's content
    // actually ends, letting a sliver of the next column bleed into view.
